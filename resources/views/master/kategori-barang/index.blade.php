        ...
        <div class="page-title-head d-flex align-items-center flex-wrap gap-2 mb-4">
            <div class="flex-grow-1">
                <h4 class="page-main-title m-0 fw-semibold">
                    <i class="ti ti-category me-2 text-primary"></i>Master Kategori Barang
                </h4>
            </div>
                ...
                <div class="datatables">
                    <div class="card shadow-sm border-0">
                        <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center">
                            <h4 class="card-title mb-0 fw-semibold"><i class="ti ti-list me-2"></i>Data Kategori Barang</h4>
                            <a href="{{ route('KategoriBarang.create') }}" class="btn btn-primary btn-sm d-flex align-items-center gap-1 px-3">
                        ...
                        <div class="card-body">
                            <div class="table-responsive">
                                <table class="table table-striped dt-responsive align-middle mb-0" id="kategoriBarangTable" width="100%">
    ...
    <script>
        $(document).ready(function() {
            ...
            $('body').on('click', '.btn-delete', function() {
                ...
                Swal.fire({
                    title: 'Hapus Data?',
                    html: `Anda akan menghapus data:<br><strong class="text-primary">${nama ? nama : 'Kategori Barang'}</strong><br>Tindakan ini tidak dapat dibatalkan!`,
                    icon: 'warning', showCancelButton: true, confirmButtonColor: '#dc3545', cancelButtonColor: '#6c757d',
                    confirmButtonText: 'Ya, Hapus!', cancelButtonText: 'Batal', reverseButtons: true
                }).then((result) => {
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "{{ route('KategoriBarang.destroy', ':id') }}".replace(':id', id),
                            type: 'DELETE',
                            ...
                            success: function(response) {
                                if (response.status === 200) {
                                    ...
                                    $('#kategoriBarangTable').DataTable().ajax.reload(null, false);
                                } else {
            ...
            // DataTables Init
            $('#kategoriBarangTable').DataTable({
                responsive: true, serverSide: true, processing: true, destroy: true,
                ajax: { url: "{{ route('KategoriBarang.index') }}", type: 'GET' },
                language: {
